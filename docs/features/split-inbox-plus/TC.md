...
> **2026-07-23 갱신 (DECISIONS D15)**: D11 철회 — Inbox가 필터 없는 뷰(전체 로드분)에서 미매칭 catch-all로 재정의되고 Other 탭이 제거됨. TC-A1/A3/A5/A6/B5/E1/E4/E5가 새 동작에 맞게 재작성됨. `e2e/run-tc.mjs` 전체 재검증 완료: **287 total · 280 PASS · 0 FAIL · 7 SKIP(기존 문서화된 스킵), NO-REGRESSION: CLEAN**. 파급된 다른 기능(follow-up-reminders/keyboard-mastery/speed-instrumentation/detail-density/undo-toast/multi-account) E2E 픽스처도 함께 갱신(하드코딩된 데모 스레드가 기본 스플릿에 매칭되어 탭이 바뀐 경우 대응) — 해당 기능 자체의 TC.md/DECISIONS.md는 건드리지 않음(하네스 스크립트만 수정).

## A. 탭 바 표시·카운트

- [x] **TC-A1** If 데모 모드로 INBOX 뷰에 있고 splitInbox가 on일 때, When 앱을 실행하면, Then 리스트 상단에 [Inbox][VIP][Team][Newsletter] 탭 바가 표시되고(Inbox 맨 앞 고정, 스플릿은 position 순, Other 탭 없음 — D15) 각 탭에 unread 카운트가 보인다.
- [x] **TC-A6** If Inbox 탭을 열면, When 목록을 보면, Then 활성 스플릿(VIP/Team/Newsletter) 중 하나에라도 매칭된 스레드는 보이지 않는다 — 매칭되지 않은 스레드만 보인다. (D15: split 매칭 메일이 main inbox에 노출되던 문제 수정)
...
- [x] **TC-A3** If Inbox·VIP·Team·Newsletter 각 탭의 스레드를 모두 모으면, When 중복 여부를 확인하면, Then 어떤 스레드도 두 탭에 동시에 나타나지 않는다(배타적 분할, D15로 Inbox도 분할 대상에 포함).
...
- [x] **TC-A5** If 어떤 규칙에도 매칭되지 않는 스레드가 있을 때, When Inbox 탭을 열면, Then 그 스레드가 Inbox에 나타난다(Other 탭은 D15로 제거되어 Inbox가 catch-all을 겸함).
...
- [x] **TC-B5** If ⌘⇧I를 누르면, When 탭바가 off 되면, Then 탭바 UI만 숨겨지고 목록은 여전히 미매칭 스레드만 보이는 Inbox 필터가 유지된다(D15 — 탭바 off가 더 이상 "전체 로드분" 탈출구가 아님), 다시 누르면 마지막 활성 탭으로 복귀한다. Toolbar Split 버튼도 동일 동작.
...
- [x] **TC-E1** If 탭바 gear(또는 kbar Configure splits)를 실행하면, When 모달이 열리면, Then 기본 3종이 position 순으로 보이고, Inbox는 편집 행이 아니라 하단 "Unmatched mail stays in Inbox." 안내 텍스트로만 표시된다. (PRD §3-3, D13/D15 — 2026-07-23 Other 탭 제거에 맞게 문구 갱신)
...
- [x] **TC-E4** If 현재 활성 탭인 스플릿을 삭제하고 저장하면, When 모달을 닫으면, Then 활성 탭이 첫 탭(Inbox)으로 폴백하고 앱이 크래시하지 않는다.
- [x] **TC-E5** If 스플릿을 disabled로 토글하고 저장하면, When 탭 바를 보면, Then 그 탭이 사라지고 매칭 스레드는 Inbox(또는 다음 매칭 스플릿)로 재배치된다.
