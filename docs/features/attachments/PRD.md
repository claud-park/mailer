...
6. 신규 TC-ATT-* E2E 전부 통과 + 기존 E2E 무회귀 — 실측 달성 **216 PASS · 0 FAIL · 5 SKIP**(baseline 200 PASS·6 SKIP + 신규 TC-ATT-* 15건, 두 clean 런에서 결정적으로 동일). SKIP이 6이 아닌 5인 이유: `TC-SA-B4`(select-all bulk-snooze 후보 판정)가 신규 데모 첨부 스레드로 인해 후보 검색 조건을 만족해 SKIP→PASS로 자연 전환 — 회귀 아님(calendar-integration DECISIONS D10과 동일 기조의 런타임 유동 SKIP). + vitest(extractAttachments·dedupeDownloadPath 순수 로직) + tsc.
