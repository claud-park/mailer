...
- **`mail:cancel-snooze`가 캐시 행에 INBOX를 즉시 반영 안 함**: 처음엔 `send&archive`와 동일하게 `applyLabelDelta(threadId, ['INBOX'], [snoozeLabel])`를 추가해 수정을 시도했으나, E2E(TC-UNDO-A4, reload 후 생존 확인)에서 **2/2 재현되는 회귀**를 만들어 되돌렸다 — 정확한 근본원인은 미확정(로컬 캐시 계층 자체(`applyLabelDelta`+`removeSnooze` 시퀀스)는 격리된 vitest로 정확함을 확인했으므로, 실제 `mail:snooze`가 만드는 캐시 상태나 `localDeltaAt`/revalidate 타이밍과의 상호작용에서 비롯된 것으로 추정). 이 항목 자체가 리뷰에서 Minor·self-healing(다음 revalidate가 provider의 fresh 상태로 자연 upsert)으로 판정됐던 만큼, 검증된 새 회귀를 감수하면서까지 밀어붙일 이유가 없어 원래 동작(캐시는 다음 revalidate가 치유)으로 롤백 — 근본원인 규명은 후속 과제로 이월.
